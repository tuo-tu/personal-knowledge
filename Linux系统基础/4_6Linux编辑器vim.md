...
```bash
Vim（Vi Improved）是 Vi（Visual Editor） 的增强版，增加了许多功能，例如语法高亮、多级撤销、插件支持等
...
​         分别是**命令模式（Command mode）**，**输入模式（Insert mode）**和**末行命令模式（Last line mode）**
...
| ------ | ------------------------ | ---------------------------------- |
| `:w`   | 保存文件                 | 保存修改，但不退出                 |
| `:w!`  | 强制保存文件             | 在只读文件或权限限制情况下强制保存 |
| `:q`   | 退出编辑器               | 文件未修改或已保存时退出           |
| `:q!`  | 强制退出编辑器           | 放弃所有修改并退出                 |
| `:wq`  | 保存文件并退出编辑器     | 保存修改并退出                     |
| `:wq!` | 强制保存文件并退出编辑器 | 强制保存修改并退出                 |
