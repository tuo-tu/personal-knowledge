...
内存逃逸是指编译器根据逃逸分析策略，将原本应该分配到**栈**上的对象分配到**堆**上的过程。

**主要逃逸场景**：

1. **返回局部变量指针**：函数返回内部变量的地址，变量必须逃逸到堆上；
2. **具体类型传给interface{}类型**：传递给interface{}参数的具体类型会逃逸，因为需要运行时类型信息；
3. **局部变量赋值给全局变量；**
4. **闭包引用外部变量**：被闭包捕获的变量会逃逸到堆上；
5. **slice、map动态扩容**：当容量超出编译期确定范围时会逃逸；
6. **大对象**：超过栈大小限制的对象直接分配到堆上；

### 10.3 内存逃逸有什么影响？

栈对象在函数结束时就被编译器回收了，而堆对象需要gc机制来释放内存，所以**大量的内存逃逸会造成gc的压力。**
...
**内存泄漏是指：**虽然 Go 有 GC机制，能自动回收不再使用的内存，但如果某些对象**仍然有引用**，即使程序逻辑上不再需要它们，GC 也不会回收，这样这些内存就会一直占用着，形成了内存泄漏。

> 简而言之：就是对象被错误地持续引用，导致 GC 无法回收。

**以下是一些内存泄漏的场景：**

- **goroutine泄漏**：goroutine没有正常退出，一直存活并占用资源。比如从channel读取数据但channel永远不会有数据写入（阻塞），或者死循环没有退出条件。我在项目中遇到过，启动了处理任务的goroutine但没有合适的退出机制，导致随着请求增加goroutine越来越多。

- **channel泄漏**：间接导致goroutine泄露。未关闭的channel和等待channel的goroutine会相互持有引用。比如生产者已经结束但没有关闭channel，消费者goroutine会一直阻塞等待，造成内存无法回收。
...
- **slice引用大数组的小部分**：当slice引用一个大数组的小部分时，整个底层数组都无法被GC回收。解决方法是使用copy创建新的slice。
- **map元素过多**：map中删除元素只是**标记删除**（对应的内存区域被标记为空闲），底层bucket不会缩减。如果map曾经很大后来元素减少，内存占用仍然很高。
- **定时器未及时stop**：`time.After`或`time.NewTimer`创建的定时器如果不手动停止，会在heap中持续存在。
...
- **pprof**：最重要的工具。
  - `go tool pprof http://localhost:port/debug/pprof/heap`，分析**堆内存分布；**
  - `go tool pprof http://localhost:port/debug/pprof/goroutine`，分析**goroutine泄漏。**
...
- 通常可以先看内存增长曲线（ pprof + 可视化图表），如果内存持续上涨不回收，就用pprof分析**哪个函数分配内存最多。**

- 如果是 goroutine 泄漏，会看到goroutine数量**异常增长**，然后分析这些goroutine阻塞在哪里。

**3、常见优化手段**：

- **goroutine泄漏**：使用context设置超时，确保goroutine有退出机制，避免无限阻塞。
- **channel泄漏**：及时关闭channel；或使用select + default避免阻塞。
- **slice引用优化**：对大数组的小slice使用copy创建独立副本。
- **定时器清理**：手动调用`timer.Stop()`停止定时器。
...
所有的 GC 算法其存在形式可以归结为：**追踪式 GC**（Tracing GC）和**引用计数**（Reference Counting, RC）这两种形式的混合运用。

**目前比较常见的实现方式有：**

- **标记清除：**从根对象出发，将确定存活的对象进行标记，并清扫可以回收的对象。
- **标记整理：**为了解决内存碎片问题而提出，在标记过程中，**将对象尽可能整理到一块连续的内存上。**
- **增量式：**将标记与清扫的过程分批执行，每次执行很小的部分，从而增量的推进垃圾回收，达到近似实时、几乎无停顿的目的。
- **增量整理：**在增量式的基础上，增加对对象的整理过程。
- **分代式：将对象根据存活时间的长短进行分类**，存活时间小于某个值的为**年轻代**，存活时间大于某个值的为**老年代**，永远不会参与回收的对象为**永久代**。并根据分代假设（如果一个对象存活时间不长则倾向于被回收，如果一个对象已经存活很长时间则倾向于存活更长时间）对对象进行回收。
- **引用计数：**根据对象自身的引用计数来回收，当引用计数归零时立即回收。

### 11.2 Go 语言的 GC 使用的是什么？

Go 的 GC 目前使用的是**无分代**（对象没有代际之分）、**不整理**（回收过程中不对对象进行移动与整理）、并发（与用户代码并发执行）的**三色标记清除算法。**

### 11.3 三色标记法是什么？

三色标记法是Go的垃圾回收器使用的核心算法。

**1、三色定义**

- **白色**：未被访问的对象，垃圾回收结束后**白色对象会被清理；**
...
- 从GC 的根对象开始，将其直达对象全部标记为灰色，并放入一个队列中，这个队列称为灰色队列；
- 然后不断从**灰色队列**中取出对象，继续扫描其引用的对象，如果引用对象是白色就标记为灰色；
- **当前对象**所有引用的对象扫描完成后，再将当前对象标记为黑色；
- 重复以上过程直到灰色队列为空。

> Root 对象是指 GC 扫描的起点集合，主要包括：**全局变量、各个 goroutine 栈上的变量以及寄存器中的指针变量。**
>
> - 每个 goroutine 都有一块**自己的栈内存**，专门用来存放函数执行过程中的**局部变量**。
> - 寄存器中的指针变量，是指 CPU 在执行过程中临时保存在寄存器里的指向堆对象的指针。

------

**提问：Go的GC只回收堆上的对象，那根对象怎么会包含goroutine 栈上的变量呢？**

答：Go 的 GC **确实只回收堆上的对象**，但它 **必须先扫描 goroutine 栈上的变量**，这是为了找到：哪些“栈上的指针”还指向堆对象。所以GC**必须扫描栈来判断堆对象是否还存活。**也就是说，根对象跟是否在堆上没有任何关系。

```
第1步：找到 GC Roots（根对象集合）
第2步：从根出发遍历堆对象
第3步：标记可达对象
第4步：回收不可达对象
```
...
1. 当垃圾回收开始时，只有白色对象；
2. 随着标记过程开始进行时，灰色对象开始出现（着色），这时候波面便开始扩大；
3. 当一个对象的所有子节点均完成扫描时，这个对象会被着色为黑色；
4. **当整个堆遍历完成时，只剩下黑色和白色对象，这时的黑色对象为可达对象，即存活；而白色对象为不可达对象，即死亡。**

这个过程可以视为以灰色对象为波面，将黑色对象和白色对象分离，使波面不断向前推进，直到所有可达的灰色对象都变为黑色对象为止的过程。如上图所示。
...
2. **执行栈中的变量**：每个 goroutine 都包含自己的执行栈，这些执行栈上包含**栈上的变量、指向堆对象的指针。**
3. **寄存器中的指针值**：寄存器的值可能表示一个指针，这些指针在执行过程中可能**指向堆上的对象。**
...
并发标记清除法的核心难点在于如何保证在用户程序**并发修改对象的引用时，垃圾回收器仍能正确识别存活对象。**

> **对象引用** 指的是**指向内存中某个对象的指针或引用变量。**
...
> 标记阶段用户程序仍在运行，对象的引用关系可能被动态修改（比如新建对象、断开引用、移动对象指针），这会导致垃圾回收器出现两种错误：
>
> - **漏标**：某个**原本不可达**的对象，在标记过程中被用户程序重新建立引用，若未被检测到，会**被误判为垃圾回收**，导致程序崩溃。
> - **错标**：某个**原本可达**的对象，在标记过程中被用户程序断开引用，若**仍被标记为存活，会造成内存泄漏。**
>
> 为了解决这个问题，CMS 引入了 **写屏障** 机制：当用户程序修改对象引用时，写屏障会拦截这个操作，并**记录下被修改的对象**，后续垃圾回收器会对这些对象重新标记，以此保证标记的准确性。
...
### 11.7 Go语言是如何解决并发标记清除时，用户程序可以并发修改对象引用的难点？

Go通过**混合写屏障**和**弱三色不变性**维护来解决这个并发安全问题。

- 核心挑战是防止"对象消失"现象：当黑色对象新增对白色对象的引用，同时灰色到白色的引用被删除时，白色对象可能被错误回收。

- Go采用**①混合写屏障**策略，在指针赋值（新建引用）时执行额外逻辑：**新建引用时将目标对象标为灰色，删除引用时将被删对象标为灰色。**这样确保关键对象不会丢失在标记过程中。

- 同时Go维护了**②弱三色不变性：允许黑色对象指向白色对象**，但要保证从白色对象反向出发时存在全灰色路径可达根对象。
...
以上这套机制让Go实现了微秒级STW时间，大部分GC工作都与用户程序并发执行，在保证回收正确性的同时将性能影响降到最低。

> 写屏障就像是一个监控，在”对象引用发生变化的时候”，帮 GC打小报告：这个对象别删，还有人在用。

### 11.8 什么是写屏障、混合写屏障，如何实现？

**1、写屏障**

写屏障的本质是在编译器在**指针赋值操作中**（也就是**引用发生变化**时），插入的额外**很短的指令**，当执行`*slot = ptr`这样的指针赋值时，写屏障会在赋值前后执行特定逻辑来**标记相关对象**，防止并发标记过程中对象被错误回收。
...
Go1.8后采用的混合写屏障，结合两者优点，在堆上建立新引用和删除引用时分别采用插入写屏障和删除写屏障。同时也做了优化，不再需要STW去重扫了。

**它的新规则是：**任何在GC标记阶段，**被创建于栈上的新对象**，都默认标记为黑色。这样一来，GC就不需要关心**栈上**的指针指向堆里的哪个白色对象了，因为**栈本身就被看作是黑色的**，它指向的对象必须是可达的。

------

**总结就是：**

- 堆上的对象：在引用发生变化时，把「旧引用指向的对象」和「新引用指向的对象」都**标灰**；
- 栈上的对象：完全不触发写屏障，在GC标记阶段，一次性把所有 Goroutine 的栈扫描**标黑；**

### 11.9 Go 语言中 GC 的流程是什么？

共分为**标记、清扫、内存归还**三个大的阶段：

清扫终止（上一轮）→标记准备→并发标记→标记终止→并发清扫→内存归还

| 阶段                   | 核心说明                                                     | 赋值器（用户程序）状态 | 写屏障状态 | 补充关键动作                                                 |
| ---------------------- | ------------------------------------------------------------ | ---------------------- | ---------- | ------------------------------------------------------------ |
| **Sweep Termination**  | 清扫终止：收尾上一轮 GC 的清扫工作；初始化本轮 GC 标记的全局状态（如标记队列、内存阈值） | STW（极短，微秒级）    | 未开启     | 1. 确保上一轮并发清扫完成；2. 计算本轮 GC 的内存触发阈值；3. 准备标记根对象 |
| **Mark Setup**         | 标记准备：GC 标记的前置准备（你漏了这个核心阶段）            | STW（极短）            | 开启       | 1. 启动混合写屏障；2. 扫描所有 Goroutine 的栈（GC Roots 之一）并标记为黑色；3. 将全局根对象（如全局变量、Goroutine 链表）入灰色队列 |
| **Concurrent Mark**    | 并发标记：遍历堆上对象，标记可达对象                         | 并发运行               | 开启       | 1. GC 线程从灰色队列取出对象，标记为黑色并将其子对象入灰色队列；2. 写屏障拦截堆引用修改，将旧 / 新引用对象标灰；3. 无栈重扫（混合写屏障核心优化） |
| **Mark Termination**   | 标记终止：确保标记周期完成，收尾标记工作                     | STW（极短，毫秒级）    | 关闭       | 1. 停止写屏障；2. 处理剩余灰色队列对象；3. 校验标记结果，确保无漏标；4. 统计存活对象 |
| **Concurrent Sweep**   | 并发清扫：释放不可达对象的内存，将内存归还给堆（你说的 “内存清扫阶段”） | 并发运行               | 关闭       | 1. 遍历堆的内存块，回收白色（未标记）对象；2. 将回收的内存标记为空闲，供后续分配使用 |
| **Concurrent Release** | 内存归还：按需将堆中多余的空闲内存归还给操作系统（你说的 “内存归还阶段”） | 并发运行               | 关闭       | 1. 仅当堆内存使用率远低于阈值时触发；2. 减少进程占用的物理内存，避免内存浪费 |
...
- **超时触发：**go后台有一个**系统监控线程（sysmon）**，当超过**两分钟**没有产生任何 GC 时，强制触发 GC。
- **内存阈值触发：**内存使用增长到一定比例时有可能会触发，每次内存分配时检查当前内存分配量是否已达到阈值（环境变量GOGC）：**默认100%**，即当内存扩大一倍时启用GC。
  - 可以通过`debug.SetGCPercent(500)`来修改阈值，这里表示如果当前堆大小超过了上次标记的堆大小的500%，就会触发GC。
  - 而第一次GC的触发的临界值是4MB。
...
内存泄漏，即预期可以被释放的内存，其**生命期意外地被延长**，因此在GC中预计能够立即回收的内存而长时间得不到回收。

Go用语言的内存泄漏主要有以下两种：

1. **内存被根对象持续引用而没有得到迅速释放：**比如某个局部变量被赋值到了一个全局变量中，这个局部变量的内存就得不到释放。
2. **goroutine 泄漏：**一些不当的使用，导致goroutine不能正常退出，也会造成内存泄漏。

### 11.13 Go 的 GC 如何调优？

1. 合理化内存分配的速度，提高内存分配器的 CPU 利用率；
2. 复用已经申请的内存 ，比如使用sync.pool复用经常需要创建的重复对象；
3. 适量将 GOGC 的值设置得更大，让 GC 触发的时间变得更晚，从而减少其触发频率，进而增加用户代码对机器的使用率。

### 11.14 如何观察 Go GC？

主要有以下几种方法：

- **实时输出 GC 极简日志：**`GODEBUG=gctrace=1`
- **追踪 GC 全流程：**`go tool trace`
- **读取 GC 核心统计：**`debug.ReadGCStats`
- **读取全量内存 / GC 指标：**`runtime.ReadMemStats`
