...
func (t *Admin) LoginCheck(c *gin.Context, tx *gorm.DB, param *dto.AdminLoginInput) (*Admin, error) {
	...
	saltPassword := common.GenSaltPassword(adminInfo.Salt, param.Password)
...
func (t *Admin) Find(c *gin.Context, tx *gorm.DB, search *Admin) (*Admin, error) {
	out := &Admin{}
	err := tx.SetCtx(util.GetGinTraceContext(c)).Where(search).Find(out).Error
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (t *Admin) Save(c *gin.Context, tx *gorm.DB) error {
	return tx.SetCtx(util.GetGinTraceContext(c)).Save(t).Error
}
