...
func (t *LoadBalance) Find(c *gin.Context, tx *gorm.DB, search *LoadBalance) (*LoadBalance, error) {
	model := &LoadBalance{}
	err := tx.SetCtx(util.GetGinTraceContext(c)).Where(search).Find(model).Error
	return model, err
}

func (t *LoadBalance) Save(c *gin.Context, tx *gorm.DB) error {
	if err := tx.SetCtx(util.GetGinTraceContext(c)).Save(t).Error; err != nil {
		return err
	}
	return nil
}
...
type LoadBalancerItem struct {
	LoadBanlance loadbalance.LoadBalance
	ServiceName  string
}
...
func (lbr *LoadBalancer) GetLoadBalancer(service *ServiceDetail) (loadbalance.LoadBalance, error) {
	...
	// 主动探测
	mConf, err := loadbalance.NewLoadBalanceCheckConf(fmt.Sprintf("%s%s", schema, "%s"), ipConf)
	if err != nil {
		return nil, err
	}
	lb := loadbalance.LoadBanlanceFactorWithConf(loadbalance.LbType(service.LoadBalance.RoundType), mConf)
