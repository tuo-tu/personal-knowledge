...
func InitDBPool(path string) error {
	...
	for confName, DbConf := range DbConfMap.List {
		...
		dbgorm.LogMode(true)
		dbgorm.LogCtx(true)
		dbgorm.SetLogger(&MysqlGormLogger{Trace: log.NewTrace()})
...
func DBPoolLogQuery(trace *log.TraceContext, sqlDb *sql.DB, query string, args ...interface{}) (*sql.Rows, error) {
	startExecTime := time.Now()
	rows, err := sqlDb.Query(query, args...)
	endExecTime := time.Now()
	if err != nil {
		log.Log.TagError(trace, "_com_mysql_success", map[string]interface{}{
			"sql":       query,
			"bind":      args,
			"proc_time": fmt.Sprintf("%f", endExecTime.Sub(startExecTime).Seconds()),
		})
	} else {
		log.Log.TagInfo(trace, "_com_mysql_success", map[string]interface{}{
			"sql":       query,
			"bind":      args,
			"proc_time": fmt.Sprintf("%f", endExecTime.Sub(startExecTime).Seconds()),
		})
...
func (logger *MysqlGormLogger) Print(values ...interface{}) {
	...
	if message["level"] == "sql" {
		log.Log.TagInfo(logger.Trace, "_com_mysql_success", message)
	} else {
		log.Log.TagInfo(logger.Trace, "_com_mysql_failure", message)
	}
}
...
func (logger *MysqlGormLogger) CtxPrint(s *gorm.DB, values ...interface{}) {
	...
	trace := log.NewTrace()
	if ok {
		trace = ctx.(*log.TraceContext)
	}
	message := logger.LogFormatter(values...)
	if message["level"] == "sql" {
		log.Log.TagInfo(trace, "_com_mysql_success", message)
	} else {
		log.Log.TagInfo(trace, "_com_mysql_failure", message)
	}
}
