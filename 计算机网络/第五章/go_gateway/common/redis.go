...
func RedisLogDo(trace *log.TraceContext, c redis.Conn, commandName string, args ...interface{}) (interface{}, error) {
	startExecTime := time.Now()
	reply, err := c.Do(commandName, args...)
	endExecTime := time.Now()
	if err != nil {
		log.Log.TagError(trace, "_com_redis_failure", map[string]interface{}{
			"method":    commandName,
			"err":       err,
			"bind":      args,
		...
		log.Log.TagInfo(trace, "_com_redis_success", map[string]interface{}{
			"method":    commandName,
			"bind":      args,
			"reply":     replyStr,
			"proc_time": fmt.Sprintf("%fs", endExecTime.Sub(startExecTime).Seconds()),
...
func RedisConfDo(trace *log.TraceContext, name string, commandName string, args ...interface{}) (interface{}, error) {
...
func RedisDefaultConfDo(commandName string, args ...interface{}) (interface{}, error) {
	c, err := RedisConnFactory("default")
	if err != nil {
		return nil, err
	}
