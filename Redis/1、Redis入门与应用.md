...
Redis一个开源的基于键值对（Key-Value）NoSQL数据库。使用ANSIC语言编写、支持网络、**基于内存但支持持久化**。性能优秀，并提供多种语言的API。

我们要首先理解一点，我们把**Redis称为KV数据库**，键值对数据库，那就可以把Redis内部的存储视为存在着一个巨大的Map，对Map的操作无非就是get和put，然后通过key操作这个key所对应的value，而这个value的类型可以多种多样，也就是Redis为我们提供的那些数据结构，比如字符串（String）、哈希(Hash)等等。
...
因为Redis的应用场景非常多，不同的公司有不同的玩法，但如果不掌握三高这条主线的话，你会遇到以下问题：
...
redis-server**加上要修改配置名和值（可以是多对）**，没有设置的配置将使用默认配置，例如：如果要用6380作为端口启动Redis，那么可以执行:
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/95e8ca2a9c284b0aaa33d224cc027bad.png" alt="image.png" style="zoom: 67%;" />
...
用redis-cli -h {host} -p {port} {command}就可以直接得到命令的返回结果，例如:
...
我们没有写-h参数，那么默认连接127.0.0.1；如果不写-p，那么默认6379端口，也就是说**如果-h和-p都没写就是连接127.0.0.1:6379这个 Redis实例。**
...
删除键，**无论值是什么数据结构类型**，del命令**都可以将其删除**。**返回删除键个数，删除不存在键返回0。**同时del命令可以支持删除多个键。
...
多少秒后过期，Redis支持对键添加过期时间,当超过过期时间后,会自动删除键，时间单位秒。

ttl命令会返回**键的剩余过期时间**，它有3种返回值：
...
> `EXPIRE` 和 `EXPIREAT` 命令都用于设置键的过期时间，但它们的区别主要在于**时间格式**：
>
> - EXPIRE 命令设置键在指定的**秒数**后过期。
>
> - EXPIREAT 命令设置键在指定的**时间戳**过期（表示自 1970-01-01 00:00:00 UTC 起的秒数）
...
3 ) persist命令可以将键的过期时间清除，使得 key 永不过期。
...
> **在 Redis 中，当你对一个带有过期时间的字符串类型键执行 `SET` 命令时，过期时间会被清除**。
>
> - Redis 的 `SET` 命令会重新创建键，并将其值更新为新值。
> - 当键被重新设置时，原有的过期时间会被清除，因为 Redis 认为这是一个全新的键。
>
> **示例：**
>
> **1.设置键并指定过期时间**：
>
> ```bash
> SET mykey "value" EX 10  # 设置键 mykey 的值为 "value"，过期时间为 10 秒
> TTL mykey                # 查看剩余生存时间
> # 输出: 10
> ```
>
...
>
> ```bash
> SET mykey "new_value"    # 覆盖键 mykey 的值
> TTL mykey                # 查看剩余生存时间
> # 输出: -1 (键变为持久化状态，无过期时间)
> ```
...
> **命名空间（Namespace）** 是一种用于组织和管理资源的概念，它的核心目的是**避免命名冲突**，并提供一种层次化的结构来分隔不同的对象或资源。
>
> **命名空间的作用**
>
> - **避免命名冲突**：多个开发者或模块可能使用相同的标识符，命名空间可以确保它们互不干扰。
> - **逻辑分组**：将相关资源分组，提高代码或资源的可读性和可维护性。
> - **权限隔离**：某些场景下，命名空间还可以用于权限管理，例如 Kubernetes 的命名空间。

**Redis没有命令空间**，而且也没有对键名有强制要求。但设计合理的键名，有利于防止键冲突和项目的可维护性，比较推荐的方式是使用**业务名:对象名: id : [属性]**作为键名（也可以不是分号）。

例如MySQL 的数据库名为mall，用户表名为order，那么对应的键可以用"mall:order:1"或者"mall:order:1:name"来表示，如果当前Redis 只被一个业务使用，甚至可以去掉“order:”。
...
- 首先**键都是字符串类型**，而且**其他几种数据结构都是在字符串类型基础上构建的**，所以字符串类型能为其他四种数据结构的学习奠定基础。
- 字符串**类型的值**实际可以是字符串(简单的字符串、复杂的字符串(例如JSON、XML))、数字(整数、浮点数)，甚至是二进制(图片、音频、视频)，但是**值最大不能超过512MB。**
...
![image.png](https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/1dab86326fa249cd9d1ab118b48e9c6a.png)
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/03c37295005c4bd19c33f7f7509778bc.png" alt="image.png" style="zoom:80%;" />
...
> 注意：
>
> - setex = set expire，表示设置过期秒数
>
> - setnx = SET if Not eXists，表示键不存在才能设置成功
> - 没有setxx的说法 

setex示例：

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/af158144826048b1ac8d2b43551d39a9.png" alt="image.png" style="zoom:67%;" />

setnx示例：

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/864f3f2d1a5a43b5a16d3e71cb808352.png" alt="image.png" style="zoom:67%;" />

因为键foo-ex已存在,所以setnx失败,返回结果为0，键foo-ex2不存在，所以setnx成功,返回结果为1。

有什么应用场景吗？以setnx命令为例子，由于Redis的单线程命令处理机制，如果有多个客户端同时执行setnx key value，根据setnx的特性只有一个客户端能设置成功，**setnx可以作为分布式锁的一种实现方案**。当然分布式锁不是只有一个命令就OK了，其中还有很多的东西要注意，我们后面会用单独的章节来讲述基于Redis的分布式锁。
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/1073c166f7364cf99ebeacd03ba3ca16.png" alt="image.png" style="zoom:80%;" />

除了incr命令，Redis还提供了decr(自减)、 incrby(自增指定数字)、decrby(自减指定数字)、incrbyfloat（自增浮点数)，具体效果请同学们自行尝试。
...
```bash
redis> GETSET db mongodb    # 没有旧值，返回 nil，并设置新值为mongodb
(nil)

redis> GET db # db的当前值是mongodb
"mongodb"

redis> GETSET db redis  # 返回旧值 mongodb，并设置新值为redis
"mongodb"

redis> GET db # db的当前值是redis
"redis"

```

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/d568dc6e0a6c4adeae13d8f63a8fd0d8.png" alt="image.png" style="zoom:80%;" />

##### setrange 设置指定位置的字符

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/bdd7efebb340433e807d61f970649218.png" alt="image.png" style="zoom:80%;" />
...
字符串这些命令中，del 、mset、 mget支持多个键的批量操作，时间复杂度和键的个数相关，为O(n)，getrange和字符串长度相关，也是O(n)，其余的命令基本上都是O(1)的时间复杂度，在速度上还是非常快的。
...
Redis 作为缓存层，MySQL作为存储层，绝大部分请求的数据都是从Redis中获取。由于Redis具有支撑高并发的特性，所以**缓存通常能起到加速读写和降低后端压力的作用。**
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/45ed460d37c04b31bbd5122cd964a0e3.png" alt="image.png" style="zoom:80%;" />
...
在使用hgetall时，如果哈希元素个数比较多，会存在阻塞Redis的可能。如果只需要获取部分field，可以使用hmget，如果一定要获取全部field-value，可以使用**hscan命令**，该命令会**渐进式遍历哈希类型**，hscan将在后面的章节介绍。

##### hincrby增加

hincrby和 hincrbyfloat，就像incrby和incrbyfloat命令一样，但是它们的**作用域是filed。**
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/4c52aa4d0f0c4841afa87011982c2151.png" alt="image.png" style="zoom:80%;" />
...
列表(list)类型是用来存储**多个有序的字符串**，a、b、c、b四个元素从左到右组成了一个有序的列表，列表中的每个字符串称为元素(element)，一个列表最多可以存储(2^32-1)个元素(*4294967295*)。
...
在Redis 中，可以**对列表两端插入( push)和弹出(pop)**，还可以获取指定范围的元素列表、获取指定索引下标的元素等。列表是一种比较灵活的数据结构，它**可以充当栈和队列的角色**，在实际开发上有很多应用场景。
...
如果命令执行成功，返回插入操作完成之后，**列表的长度**。 如果没有找到指定元素（value） ，返回 -1 。 如果 key 不存在或为空列表，返回 0 。
...
这三个返回结果为命令完成后当前列表的长度，也就是**列表中包含的元素个数**，同时rpush和lpush都支持同时插入多个元素。

##### lpop 从列表左侧弹出（会删除元素）

![image.png](https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/cf3b7a2f1d85434689734dea9d467c0f.png)
...
> 也就是说，列表只保留指定区间内的元素，不在指定区间之内的元素都将被删除。
...
![image.png](https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/8e52841e88f34a05888da5df0a8a4121.png)
...
![image.png](https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/f0b6ab8eca224305bcd97a4916c33947.png)
...
blpop和brpop是lpop和rpop的**阻塞版本**，除此之外还支持多个列表类型，也支持设定阻塞时间（超时时间），单位秒，**如果阻塞时间为0，表示（若没有元素）会一直阻塞下去**。我们以brpop为例说明。

>  BLPOP 命令如果指定的弹出列表不存在或者为空，会一直等待到超时时间返回(nil)或者一直等到列表中被其他链接插入元素时，再弹出返回。当List有值的时候，会立刻pop出来。
>
> BLPOP 命令最后一个参数 timeout 是超时时间，单位是秒，如果 timeout 大于0，则到达指定的秒数即使没有弹出成功也会返回（nil），如果 timeout 的值为0，则会一直阻塞等待其他连接向列表中插入元素， timeout 参数不允许为负数。
...
消息队列，Redis 的 lpush+brpop命令组合即可实现**阻塞队列**，生产者客户端使用lpush从列表左侧插入元素，多个消费者客户端使用brpop命令阻塞式的“抢”列表尾部的元素,多个客户端保证了消费的负载均衡和高可用性。

**文章列表**

每个用户有属于自己的文章列表，现需要分页展示文章列表。此时可以考虑使用列表,因为**列表不但是有序的,同时支持按照索引范围获取元素。**
...
集合( set）类型也是用来保存多个的字符串元素，但**和列表类型不一样的是，集合中不允许有重复元素**，并且集合中的元素是无序的,不能通过索引下标获取元素。
...
集合类型比较典型的使用场景是**标签(tag)**。例如一个用户可能对娱乐、体育比较感兴趣，另一个用户可能对历史、新闻比较感兴趣，这些兴趣点就是标签。有了这些数据就可以得到喜欢同一个标签的人，以及用户的共同喜好的标签，这些数据对于用户体验以及增强用户黏度比较重要。
...
有序集合提供了获**取指定分数和元素范围查询、计算成员排名等**功能，合理的利用有序集合，能帮助我们在实际开发中解决很多问题。
...
- WEIGHTS weight [weight ...]：指定每个有序集合的权重，在做交集计算时，每个键中的每个member 会**将自己分数乘以这个权重**，每个键的权重默认是1。


- aggregate sum|min|max：指定交集结果的聚合方式，分值可以按照sum(和)、min(最小值)、max(最大值)做汇总，**默认是sum。**

不太好理解，我们用一个例子来说明。（算平均分）

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/cfc3967b70cb4dcea41057c3708d8616.png" alt="image.png" style="zoom:80%;" />
...
该命令的所有参数和zinterstore是一致的，只不过是做**并集计算**，大家可以自行实验。
...
有序集合比较典型的使用场景就是**排行榜系统**。例如视频网站需要对用户上传的视频做排行榜，榜单的维度可能是多个方面的：**按照时间、按照播放数量、按照获得的赞数。**按照多个维度累计最终的排行榜。
...
Bitmaps单独提供了一套命令，所以在Redis中使用Bitmaps和使用字符串的方法不太相同。可以把 Bitmaps想象成一个**以位为单位的数组**，数组的**每个单元只能存储0和1**，数组的下标在 Bitmaps 中叫做**偏移量**。
...
设置键的第 offset 个位的值(从0算起)。只能设置为0或1

实例：

```bash
redis> SETBIT bit 10086 1 # 将10086位置设置为1
(integer) 0
...
假设现在有20个用户（对应20个位），userid=0,2,4,6,8的用户对网站进行了访问，存储键名为日期。
...
获取键的第offset位的值(从0开始算)，比如获取userid=8的用户是否在2022（年/这天）访问过,返回0说明没有访问过:
...
```bash
bitop and destkey key1 key2 ... keyN #对多个key求逻辑并，并将结果保存到destkey
bitop or destkey key1 key2 ... keyN #多个key求逻辑或，并将结果保存到destkey
bitop xor destkey key1 key2 ... keyN #对多个key求逻辑异或，并将结果保存到destkey
...
```bash
SET key1 "a"          # ASCII 'a': 01100001
SET key2 "b"          # ASCII 'b': 01100010

BITOP AND result key1 key2
GET result            # 返回：`!` (ASCII: 01100000)
```

##### bitpos 计算Bitmaps（的某个范围中）第一个值为targetBit 的偏移量
...
>
> 这个命令有一个奇怪的问题，就是查找0和1的处理过程有些不同，因为所有的字符串都可以认为在末尾有一个0，所以 BITPOS 命令在字符串中找1可能找不到，但是找0一定可以找得到，如果到末尾还没有找到0，则会返回**末尾后一个位置**，因为字符串默认结尾后面都是0，如果**找1找不到就会返回-1。**

计算0815当前访问网站的最小用户id（？？？？）

除此之外，bitpos有两个选项[start]和[end]，分别代表起始字节和结束字节。
...
> 布隆过滤器（**Bloom Filter**）是一种高效的**概率型数据结构**，用于**判断一个元素是否可能存在于集合中。**
>
> **核心特点**
>
> **优点**：
>
> - **空间效率高**：相比于直接存储所有元素，布隆过滤器能显著减少内存使用。
> - **查询速度快**：利用多个哈希函数并行工作，时间复杂度为 O(k)，其中 k 是哈希函数的数量。
>
> **缺点**：
>
> - **存在误判**：可能会误认为不存在的元素在集合中（**假阳性**）。
> - **不可删除元素**：标准布隆过滤器无法删除已添加的元素。
>
> **基本原理**
>
> 1. **初始化**：
>    - 创建一个长度为 m 的位数组（bit array），初始值全为 0。
>    - 定义 k 个独立的哈希函数。
> 2. **添加元素**：
>    - 对元素进行 k 次哈希计算，得到 k 个哈希值，对应位数组中的 k 个位置置为 1。
> 3. **查询元素**：
>    - 对查询元素进行 k 次哈希计算，检查对应的 k 个位置是否均为 1。
>    - 如果全为 1，则可能在集合中（有误判可能）；否则，确定不在集合中。

**布隆过滤器实现：**这里使用了一个hash函数，但是需要使用hashCounts次。
...
type BloomFilter struct {
    size       uint // 位数组大小
    hashCounts uint // 哈希函数数量
    bitArray   []bool // 布隆过滤器的位数组
}

// 创建布隆过滤器
func NewBloomFilter(size uint, hashCounts uint) *BloomFilter {
    return &BloomFilter{
       size:       size,
...
func hash(value string, seed uint) uint {
    h := fnv.New32a()
    h.Write([]byte(value)) // 将value写入到哈希对象h中
    return uint(h.Sum32()) + seed
}

// 添加元素到布隆过滤器
func (bf *BloomFilter) Add(item string) {
    for i := uint(0); i < bf.hashCounts; i++ { //有几个哈希函数就循环几次
       index := hash(item, i) % bf.size // 获取需要设置true的位的下标位置
       bf.bitArray[index] = true // 将对应的位设置为true
    }
}

// 检查元素是否可能存在
func (bf *BloomFilter) Check(item string) bool {
    for i := uint(0); i < bf.hashCounts; i++ {
       index := hash(item, i) % bf.size
       if !bf.bitArray[index] { // 一旦发现对不上，即刻结束，表明元素一定不存在
          return false
       }
    }
    return true
...
func main() {
    ...
    // 检查名字是否存在
    toCheck := []string{"Alice", "Eve", "Charlie", "Frank"}
    for _, name := range toCheck {
       if bf.Check(name) {
          fmt.Printf("%s might exist\n", name)
       } else {
...
> 注意：哈希函数生成的值通常是一个 32 位或 64 位整数，可能远远超过布隆过滤器位数组的长度（`size`）。为了避免超出数组边界，需要通过 **取模运算** 将哈希值映射到数组的有效范围内。（见代码）

1970 年布隆提出了一种**布隆过滤器**的算法，用来**判断一个元素是否可能在一个集合中**。这种算法由**一个二进制数组和一个 Hash 算法组成。**

本质上布隆过滤器是一种数据结构，比较巧妙的概率型数据结构（probabilistic data structure），特点是**高效地插入和查询**，可以用来告诉你 **某样东西一定不存在或者可能存在**。

相比于传统的 List、Set、Map 等数据结构，它更高效、占用空间更少，但是缺点是**其返回的结果是概率性的，而不是确切的。**
...
- 本质是**hash冲突**，即哈希函数存在两个输入有同一个输出值的情况（但是不存在一对多）
...
如果你负责开发维护一个大型的网站，有一天产品经理要网站每个网页每天的 **UV（Unique Visitor独立访客数据）**，然后让你来开发这个统计模块，你会如何实现？

如果统计 **PV（页面浏览量）** 那非常好办，给每个网页一个独立的 Redis 计数器就可以了，这个计数器的 key 后缀加上当天的日期。这样来一个请求，incrby 一次，最终就可以统计出所有的 PV 数据。
...
pfadd用于向HyperLogLog 添加元素,如果添加成功返回1（如果至少有个元素被添加返回 1， 否则返回 0）:
...
pfcount用于计算（估算）一个或多个HyperLogLog的独立总数，例如u-9-30 的独立总数为8：
...
pfmerge可以求出多个HyperLogLog的**并集**并赋值给destkey，请自行测试。
...
马老师说大概你抛了128个回合（估算n=2的7次方）。这个是怎么算的。

k是每回合抛到1所用的次数，我们已知的是最大的k值，可以用kmax表示。由于每次抛硬币的结果只有0和1两种情况，因此，能够推测出kmax在任意回合出现的概率 ，并由kmax结合极大似然估算的方法推测出n的次数n = 2^(k_max) 。概率学把这种问题叫做伯努利实验。但是问题是，这种本身就是概率的问题，我跟马老师说，我只用到12次，并且有视频为证。

所以这种预估方法存在较大误差，为了改善误差情况，HLL中引入分桶平均的概念。

同样举抛硬币的例子，如果只有一组抛硬币实验，显然根据公式推导得到的实验次数的估计误差较大；如果100个组同时进行抛硬币实验，受运气影响的概率就很低了，**每组分别进行多次**抛硬币实验，并上报各自实验过程中抛到正面的抛掷次数的最大值，就能根据100组的平均值预估整体的实验次数了。
...
怎么分？根据得到的比特串的**后14位**来做判断即可。

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1663252342001/bd71287e85294b14b95e3fcb82243fab.png" alt="image.png" style="zoom: 67%;" />
...
16384*6 /8/1024=12k。并且这里最多可以存储多少数据，因为是64位吗，所以就是2的64次方的数据，这个存储的数据非常非常大的，一般用户用long来定义，最大值也只有这么多。（注意，这里可以正确描述为，通过12k的存储空间，就可以推断出 2^64那么大的统计数字）。解释：

**64 位哈希函数的统计能力**

**统计范围**：

- 使用 64 位哈希函数，每个输入值会被映射到 [0, 2^64) 的范围。对应上面的例子，也就是说不同的userid可以有2的64次方个。因为pfcount本来就是统计key的不同值得个数。
- 理论上可以表示高达 2^64 个不同值（即不同的userid）。

> 自我总结：假设有数以亿计的用户，每个用户的userid经过hash函数计算都会得到一个二进制字符串，每个字符串从右往左数，找到第一个1出现的位置，1后面有很多0，假设0的数量是n。在这数以亿计的用户中，总有一个最大的n（假设记为max_n）。我们不难推断，用户的数量越多，这个max_n的值就会越大（至少理论上是这样的），也就是说，用户的数量和这个max_n之间有着某种联系（虽然不是必然的联系），那我们现在就可以用max_n来大致推断出用户的数量啦！（计算的公式暂时不用考虑）。
...
然后把每个桶的值做调和平均数，就可以算出一个算法值。同时，在具体的算法实现上，HLL还有一个分阶段偏差修正算法。我们就不做更深入的了解了。
...
业界比较通用的地理位置距离排序算法是**GeoHash 算法**，Redis 也使用GeoHash算法。**GeoHash算法将二维的经纬度数据映射到一维的整数**，这样所有的元素都将在挂载到一条线上，距离靠近的二维坐标映射到一维后的点之间距离也会很接近。当我们想要计算「附近的人时」，首先将目标位置映射到这条线上，然后在这个一维的线上获取附近的点就行了。
...
- 如果需要**更新地理位置信息**，仍然可以使用geoadd命令，虽然返回结果为0。
- geoadd命令可以同时**添加多个**地理位置信息:
...
>
> georadiusbymember 和 GEORADIUS 命令一样， 都可以找出位于指定范围内的元素， 但是 georadiusbymember 的中心点是**由给定的位置元素**决定的， 而不是使用经度和纬度来决定中心点。
...
georadius和georadiusbymember两个命令的作用是一样的，都是以一个地理位置为中心算出指定半径内的其他地理信息位置，不同的是

- georadius命令的中心位置给出了具体的经纬度；

- georadiusbymember只需给出成员即可。

其中radius  m | km |ft |mi是**必需参数**，指定了半径(带单位)。
...
Redis使用geohash**将二维经纬度转换为一维字符串**，下面操作会返回beijing的geohash值。
