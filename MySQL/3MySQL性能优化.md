...
- 对于**MySQL调优**，需要确认**业务表结构设计是否合理，SQL语句优化是否足够**，该添加的索引是否都添加了，是否可以剔除多余的索引等等


- 对于**硬件和OS调优**，需要对硬件和OS有着非常深刻的了解，仅仅就磁盘一项来说，一般非DBA能想到的调整就是SSD盘比用机械硬盘更好。DBA级别考虑的至少包括了，使用什么样的磁盘阵列（RAID）级别、是否可以分散磁盘IO、是否使用裸设备存放数据，使用哪种文件系统（目前比较推荐的是XFS），操作系统的磁盘调度算法选择，是否需要调整操作系统文件管理方面比如atime属性等等。


所以本章**我们重点关注MySQL方面的调优，特别是索引。**SQL/索引调优要求对业务和数据流非常清楚。在阿里巴巴内部，有三分之二的DBA是业务DBA，从业务需求讨论到表结构审核、SQL语句审核、上线、索引更新、版本迭代升级，甚至哪些数据应该放到非关系型数据库中，哪些数据放到数据仓库、搜索引擎或者缓存中，都需要这些DBA跟踪和复审。他们甚至可以称为数据架构师（Data Architecher）。

### 1.3.2.查询性能优化

前面的章节我们知道如何设计最优的库表结构、如何建立最好的索引，这些对于高性能来说是必不可少的。但这些还不够，还需要**合理的设计查询**。如果查询写得很糟糕，即使库表结构再合理、索引再合适，也无法实现高性能。

#### 1.3.2.1.什么是慢查询

慢查询日志，顾名思义，就是查询花费大量时间的日志，是指MySQL记录的所有执行超过**long_query_time**参数设定的时间阈值的SQL语句的日志。该日志能为SQL语句的优化带来很好的帮助。默认情况下，慢查询日志是关闭的，**要使用慢查询日志功能，首先要开启慢查询日志功能**。如何开启，我们稍后再说。
...
有些查询会请求实际不需要的数据，然后这些多余的数据会被应用程序丢弃。这会给MySQL服务器带来额外的负担，并增加网络开销，另外也会消耗应用服务器的CPU和内存资源。比如:

- **查询不需要的记录**

一个常见的错误是常常会误以为MySQL会只返回需要的数据，实际上MySQL却是先返回全部结果集再进行计算。我们经常会看到一些了解其他数据库系统的人会设计出这类应用程序。

解释：“从 `order_exp` 表中，跳过前 10,000 行，获取接下来的 20 行数据。”

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/c42215b7f5c442869027906f5aa80f4e.png" alt="image.png" style="zoom:80%;" />

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/8915ae332c7b4dbea0fa3c591e902197.png" alt="image.png" style="zoom:80%;" />

以上SQL你认为MySQL会执行查询，并只返回他们需要的20条数据，然后停止查询。**实际MySQL 仍然需要扫描并跳过前面的 10000 条记录，然后才能开始返回接下来的 20 条记录。**

- **总是取出全部列**

每次看到`SELECT *`的时候都需要用怀疑的眼光审视，是不是真的需要返回全部的列？很可能不是必需的。取出全部列，会让优化器无法完成索引覆盖扫描这类优化，还会为服务器带来额外的I/O、内存和CPU的消耗。因此，一些DBA是严格禁止`SELECT *`的写法的，这样做有时候还能避免某些列被修改带来的问题。
...
在EXPLAIN语句中的type列反应了访问类型。访问类型有很多种，从**全表扫描到索引扫描、范围扫描、唯一索引查询、常数引用等**。这里列的这些，速度是从慢到快，扫描的行数也是从小到大。你不需要记住这些访问类型，但需要明白扫描表、扫描索引、范围访问和单值访问的概念。
...
1. 在索引中使用WHERE条件来过滤不匹配的记录。这是在**存储引擎层**完成的。(最快)

2. 使用**覆盖索引**扫描来返回记录，直接从索引中过滤不需要的记录并返回命中的结果。这是在 MySQL服务器层完成的，但无须再回表查询记录。（中等速度）
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/f72a83180181494fa68d4f014375127b.png" alt="image.png" style="zoom:80%;" />
...
但是多慢算慢？MySQL中可以设定一个阈值，将运行时间超过该值的所有SQL语句都记录到慢查询日志中。`long_query_time`参数就是这个阈值。**默认值为10，代表10秒**。
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/19a8d15f0d724fae93946b5bd4f63cc3.png" alt="image.png" style="zoom:80%;" />

开启1，关闭0（默认）

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/67a74308e0524b459cb30cfa66bdba40.png" alt="image.png" style="zoom: 67%;" />
...
```sql
SET GLOBAL long_query_time = 2;  -- 设置 2 秒为慢查询阈值
```

2. `slow_query_log_file` **指定慢查询日志的存储路径及文件（默认和数据文件放一起）**

```sql
SET GLOBAL slow_query_log_file = '/var/log/mysql_slow.log';
```

3. `long_query_time` 指定记录慢查询日志SQL执行时间的阈值（单位：秒，默认10秒）
...
```sql
SET GLOBAL log_queries_not_using_indexes = ON; -- ON（即全表扫描 Full Table Scan）
```

5. log_output 选择日志存储方式，日志的存储方式可以是

   - **`FILE`**：日志存入文件（默认）

   - **`TABLE`**：日志存入 MySQL `mysql.slow_log` 表
...
chatgpt：`EXPLAIN` 用于分析 `SELECT` 查询的执行计划，帮助我们**优化 SQL 语句，提高查询性能**。

有了慢查询语句后，就要对语句进行分析。一条查询语句在经过MySQL查询优化器的各种基于成本和规则的优化会后生成一个所谓的**执行计划**，**这个执行计划展示了接下来具体执行查询的方式**，比如多表连接的顺序是什么，对于每个表采用什么访问方法来具体执行查询等等。EXPLAIN语句来帮助我们查看某个查询语句的具体执行计划，我们需要搞懂EPLATNEXPLAIN的各个输出项都是干嘛使的，从而可以有针对性的提升我们查询语句的性能。

通过**使用EXPLAIN关键字可以模拟优化器执行SQL查询语句**，从而知道MySQL是如何处理你的SQL语句的。分析查询语句或是表结构的性能瓶颈，总的来说通过EXPLAIN我们可以：
...
重点的就是**EXPLAIN后面你要分析的SQL语句**

除了以SELECT开头的查询语句，其余的**DELETE、INSERT、REPLACE以及UPOATE语句前边都可以加上EXPLAIN**，用来查看这些语句的执行计划，不过我们这里对SELECT语句更感兴趣，所以后边只会以SELECT语句为例来描述EsxPLAIN语句的用法。

```sql
EXPLAIN SELECT * FROM test WHERE user_name = 'Tom'; -- 普通模式
EXPLAIN FORMAT=JSON SELECT * FROM test WHERE user_name = 'Tom'; --JSON模式
```

#### 1.3.4.3.执行计划详解

为了让大家先有一个感性的认识，我们把EXPLAIN语句**输出的各个列**的作用先大致罗列一下:
...
| 字段名          | 作用                                                         |
| --------------- | ------------------------------------------------------------ |
| `id`            | 查询的**执行顺序**，`id` 相同表示并行执行，不同表示按顺序执行（每个SELECT关键字都对应一个唯一的id） |
| `select_type`   | 查询类型，如 `SIMPLE`（简单查询）、`PRIMARY`（主查询）、`SUBQUERY`（子查询）等 |
| `table`         | 查询的表                                                     |
| `partitions`    | 查询使用的分区（仅适用于分区表）                             |
| `type`          | 针对单表的访问方法（连接类型），**越靠近 `ALL`，性能越差**   |
| `possible_keys` | **可能使用的索引**，但不一定真的用到了                       |
| `key`           | **实际使用的索引**，如果为 `NULL`，说明没有使用索引          |
| `key_len`       | 实际使用到的**索引长度（字节数）**，值越小越好               |
| `ref`           | 当使用索引列等值查询时，与索引列进行等值匹配的对象信息，如 `const`、`NULL`、`func` 等 |
| `rows`          | **MySQL 预估需要扫描的行数**，值越大，查询越慢               |
| `filtered`      | 某个表经过搜索条件 `WHERE` 过滤后剩余记录条数的百分比        |
| `Extra`         | **额外信息**，如 `Using index`（覆盖索引）、`Using filesort`（外部排序，性能差）等 |

**partitions**：分区表是 MySQL 提供的一种**将一张大表水平切分为多个逻辑子表**的功能（这些子表称为“分区”）
...
| `ref` 值      | 含义                                                     | 示例                                                         |
| ------------- | -------------------------------------------------------- | ------------------------------------------------------------ |
| `const`       | 使用常量进行索引查找（如 `id = 1`）                      | `WHERE id = 1`                                               |
| `NULL`        | 没有使用索引（`key` 也是 NULL）                          | 全表扫描                                                     |
| `func`        | 使用了函数（如 `WHERE YEAR(time)=2022`）无法直接使用索引 | `WHERE YEAR(created_at)=2022`                                |
| `field_name`  | 使用某个字段与索引匹配                                   | 通常出现在 JOIN 中，`orders.user_id = users.id`，则 `ref=users.id` |
| `const,const` | 多列联合索引同时使用多个常量                             | `WHERE a = 1 AND b = 2`（联合索引）                          |
...
比如下边这个查询语句中就包含2个SELECT关键字（也可能不是）:
...
比如下边这个查询中只有一个SELECT关键字，所以EXPLAIN的结果中也就**只有一条id列为1**的记录∶
...
可以看到，上述连接查询中参与连接的几个表分别对应一条记录，但是这几条记录对应的id值都是1。这里需要大家记住的是，在连接查询的执行计划中，每个表都会对应一条记录，这些记录的**id列的值是相同的**。
...
**子句会把多个查询的结果集合并起来并对结果集中的记录进行去重**，怎么去重呢? MySQL使用的是内部的临时表。正如上边的查询计划中所示，**UNION 子句是为了把id为1的查询和id为2的查询的结果集合并起来并去重**，所以在内部创建了一个**名为&#x3c;union1，2>的临时表**（就是执行计划第三条记录的table列的名称)，**id为NULL表明这个临时表是为了合并两个查询的结果集而创建的**。

跟UNION 对比起来，**UNION ALL就不需要为最终的结果集进行去重**，它只是**单纯的把多个查询的结果集中的记录合并成一个并返回给用户**，所以也就不需要使用临时表。所以在包含UNION ALL子句的查询的执行计划中，就没有那个id为NULL的记录，如下所示:
...
和分区表有关，**一般情况下**我们的查询语句的执行计划的partitions列的值**都是NULL**。

##### type

我们前边说过**执行计划的一条记录**就代表着MySQL**对某个表的执行查询时**的访问方法/访问类型，其中的type列就表明了这个访问方法/访问类型是个什么东西，是较为重要的一个指标，结果值**从最好到最坏**依次是：

出现比较多的是：`system > const > eq_ref > ref> range > index > ALL`

| `type` 值 | 含义简述                                                  | 性能   |
| --------- | --------------------------------------------------------- | ------ |
| `system`  | 表只有一行（系统表），效率最高。                          | 🔥 极高 |
| `const`   | 通过主键或唯一辅助索引等值查询，**仅返回一行结果**。      | 🔥 极高 |
| `eq_ref`  | 用于 JOIN 中，主键或唯一辅助索引等值匹配，一对一。        | 🔥 很高 |
| `ref`     | 用于非唯一索引等值查询，可能返回多行。                    | ✅ 较好 |
| `range`   | 使用索引范围查询，如 `BETWEEN`、`>`、`<`、`LIKE 'abc%'`。 | ✅ 较好 |
| `index`   | 全索引扫描（不回表），效率较低。                          | ⚠️ 一般 |
| `ALL`     | 全表扫描，性能最差。                                      | ❌ 差   |
...
当**表中只有一条记录**并且该表使用的存储引擎的统计数据是精确的，比如MyISAM、Memory，那么对该表的访问方法就是system。

```sql
explain select * from test_myisam;
```

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/0cd60414ae624c2aad6b7870d8702116.png" alt="image.png" style="zoom:80%;" />

当然，如果改成使用InnoDB存储引擎，试试看执行计划的type列的值是什么。

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/5f326bb8471841da9b8b750877c497b8.png" alt="image.png" style="zoom:80%;" />

###### const

就是当我们根据**主键或者唯一辅助索引列**与常数进行等值匹配时，对单表的访问方法就是const。**因为只匹配一行数据，所以很快**。
...
B+树叶子节点中的记录是按照索引列排序的，对于的聚集索引来说，它对应的B+树叶子节点中的记录就是按照id列排序的。B+树矮胖，所以这样根据主键值定位一条记录的速度很快。类似的，我们根据唯一辅助索引列来定位一条记录的速度也很快的，比如下边这个查询：
...
这个查询的执行分两步，第一步先从`u_idx_day_status`对应的B+树索引中根据索引列与常数的等值比较条件定位到一条辅助索引记录，然后再根据该记录的id值到聚集索引中获取到完整的用户记录。

**MySQL把这种通过主键或者唯一辅助索引列来定位一条记录的访问方法定义为：const**，意思是**常数级别**的，代价是可以忽略不计的。

不过这种const访问方法**只能在主键列或者唯一辅助索引列和一个常数进行等值比较时才有效**，如果是联合主键/联合唯一辅助索引，需要 **所有参与索引的列都进行等值比较**。

注意：`IS NULL` 查询在唯一辅助索引下**不具备这个保证**：对于唯一辅助索引来说，查询该列为NULL值的情况比较特殊，因为唯一二级索引列并不限制 NULL 值的数量，因此查询 `WHERE col IS NULL` 时，**可能命中多行**，也就是说is null不可以使用const访问方法来执行。

###### eq_ref

在**连接查询**时，如果**被驱动表**是通过**主键或者唯一辅助索引**列等值匹配的方式进行访问的〈如果该主键或者唯一辅助索引是联合索引的话，所有的索引列都必须进行等值比较)，则对该被驱动表的访问方法就是eq_ref。

(驱动表与被驱动表：A表和B表通过`join`连接查询，如果通过A表的结果集作为循环基础数据，然后一条一条地通过该结果集中的数据作为过滤条件到B表中查询数据，然后合并结果。那么我们称A表为驱动表，B表为被驱动表)

chatgpt：`eq_ref` 表示：**对于驱动表中的每一条记录，被驱动表通过唯一索引或主键查找，最多只返回一条匹配记录。**

- 是 **一种等值连接**
- **索引必须是唯一的（UNIQUE 或 PRIMARY KEY）**
- 且是 **等值条件**（比如 `=` 连接）
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/a33c7b8ce8e449d79866fc374baa7739.png" alt="image.png" style="zoom:80%;" />

从执行计划的结果中可以看出（经过了转换）：

- `s2` 是驱动表
- `s1` 是被驱动表
- `s1.id` 是主键，且 `s1.id = s2.id` 是等值连接
- 所以 `s1` 表的访问类型是 **`eq_ref`**

###### ref

chatgpt：MySQL **使用了普通索引**（非唯一索引或非主键），**使用等值匹配查找**，可能匹配多行。

当通过普通的二级索引列（非唯一索引或非主键）与常量进行等值匹配时来查询某个表，那么对该表的访问方法就可能是ref。本质上也是一种索引访问，它返回所有匹配某个单独值的行，然而，**它可能会找到多个符合条件的行**，所以他属于查找和扫描的混合体。
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/adbe165526944750b5087866b270c858.png" alt="image.png" style="zoom:80%;" />
...
chatgpt：`range` 表示 **使用索引进行范围扫描**，MySQL 会根据条件，扫描匹配的那一部分索引范围。

如果**使用索引获取某些范围区间的记录**，那么就可能使用到range访问方法，一般就是在你的**where语句中**（注意与查询列无关）出现了between、&#x3c;、>、in等的查询。这种**范围扫描索引**比全表扫描要好，因为它**只需要开始于索引的某一点，而结束于另一点，不用扫描全部索引。**
...
此处所说的使用索引进行范围匹配中的 **`索引` 可以是聚集索引，也可以是二级索引。**

###### index（与查询列有关）

> chatgpt：MySQL **扫描整个索引树** 来获取数据，**不会回表**（因为数据已经在索引中了）。
>

当我们可以**使用覆盖索引，但需要扫描全部的索引记录**时，该表的访问方法就是index。

注意：覆盖索引表示查询的 SELECT 列、WHERE 条件列、ORDER BY 列等必须全部包含在同一个索引中。

```sql
EXPLAIN
SELECT insert_time FROM s1 WHERE expire_time = '2021-03-22 18:36:47';
-- 需要在insert_time、expire_time上创建覆盖索引
```
...
查询只用到了 `age`，那就不需要回表，从 `idx_age` 中**顺序读取**就行了。

###### all

最熟悉的**全表扫描**，将遍历全表以找到匹配的行。
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/315c3953f3a64e249435e19a5ce2089f.png" alt="image.png" style="zoom:80%;" />

##### possible_keys与key

在EXPLAIN 语句输出的执行计划中，`possible_keys`列表示在某个查询语句中，对某个表执行单表查询时**可能用到的索引**有哪些，`key`列表表示**实际用到的索引**有哪些，如果为NULL，则没有使用索引。比方说下边这个查询:。
...
上述执行计划的possible keys列的值表示该查询可能使用到`u_idx_day_status`，`idx_insert_time`两个索引，然后key列的值是`u_idx_day_status`，**表示经过查询优化器计算使用不同索引的成本后，最后决定使用u_idx_day_status来执行查询比较划算。**

##### key_len

**`key_len`列表示当优化器决定使用某个索引执行查询时，该索引记录的最大长度**，计算方式是这样的：

- 对于使用**固定长度类型的索引列**来说，它实际占用的存储空间的最大长度就是该固定值，

- 对于指定字符集的**变长类型的索引列**来说，比如**某个索引列的类型是VARCHAR(100)**，使用的字符集是utf8，那么该列实际占用的最大存储空间就是100 x 3 = 300个字节（每个字符最多占 **3个字节（UTF-8）**）。

**如果该索引列可以存储NULL值**，则key_len比不可以存储NULL值时多1个字节。**对于变长字段来说，都会有2个字节的空间来存储该变长列的实际长度。**
...
由于order_no列的类型是VARCHAR(50)，所以该列实际最多占用的存储空间就是50*3字节，又因为该列是可变长度列，所以key_len需要加2（这是不确定的），所以最后ken_len的值就是152。
...
| ---- | ----- | ---- | ------------ | ------- | ---- | ----------- |
| 1    | user  | ref  | idx_name_age | 63      | 10   | Using where |
...
- `name` 是 `VARCHAR(20)`
- 每个字符最多占 **3个字节（UTF-8）**
- `VARCHAR` 类型还多了 **1~2 个字节的长度信息**
- 所以 `20 × 3 + 1 = 61` 字节左右，再加上对齐/系统字节，MySQL可能按 63 来处理
...
那就用了 `name + age` 两列（联合索引被完全使用），`key_len` 会变成 `63 + 4 = 67`（`age` 是 `INT`，占 4 个字节）。

✅ 你可以根据 `key_len` 来判断：

| key_len 值               | 说明                                |
| ------------------------ | ----------------------------------- |
| 较短（只用到索引一部分） | 联合索引中 **后面的字段没有被使用** |
| 值与联合索引列总长度一致 | **联合索引被完整使用**              |
| 值为 NULL                | 没有使用索引（索引未命中）          |

MySQL在执行计划中输出key_len列主要是为了让我们**区分某个使用联合索引的查询具体用了几个索引**列（复合索引有最左前缀的特性，如果复合索引能全部使用上，则是复合索引字段的索引长度之和，这也可以用来判定复合索引是否部分使用，还是全部使用)，而不是为了准确的说明针对某个具体存储引擎存储变长字段的实际长度占用的空间到底是占用1个字节还是2个字节。

##### rows

如果查询优化器决定使用**全表扫描**的方式对某个表执行查询时，执行计划的rows列就代表**预计需要扫描的行数**，如果**使用索引**来执行查询时，执行计划的rows列就代表**预计扫描的索引记录行数**。比如下边两个个查询：
...
chatgpt：MySQL 预估扫描的记录中，有多少百分比能通过 WHERE 条件过滤下来。

举例：如果 `rows = 1000`，`filtered = 10.0`，表示预计约 **100 条记录** 能通过 WHERE 条件。
...
从执行计划的key列中可以看出来，该查询使用 PRIMARY key来执行查询，从rows列可以看出满足id > 5890的记录有5286条。执行计划的filtered列就代表查询优化器预测在这5286条记录中，有多少条记录满足其余的搜索条件，也就是order_note = 'a'这个条件的百分比。此处filtered列的值是10.0，说明查询优化器预测在5286条记录中有10.00%的记录满足order_note = 'a'这个条件。

对于单表查询来说，这个filtered列的值没什么意义，我们更关注在连接查询中驱动表对应的执行计划记录的filtered值，比方说下边这个查询:

```sql
EXPLAIN 
SELECT * FROM s1 INNER JOIN s2 ON s1.order_no = s2.order_no WHERE s1.order_note > '你好，李焕英';
```
...
从执行计划中可以看出来，查询优化器打算把s1当作驱动表，s2当作被驱动表。我们可以看到驱动表s1表的执行计划的rows列为10573，filtered列为33.33 ，这意味着驱动表s1的扇出值就是10573 x 33.33 % = 3524.3，这说明还要对被驱动表执行大约3524次查询。
...
3.优化：优化SQL语句，例如**重写查询，决定表的读取顺序**，以及**选择需要的索引等**。这一阶段用户是可以查询的，查询服务器优化器是如何进行优化的，便于用户重构查询和修改相关配置，达到最优化。这一阶段还涉及到存储引擎，优化器会询问存储引擎，比如某个操作的开销信息、是否对特定索引有查询优化等。
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/5341da5a2f8c4d548496e938571446fe.png" alt="image.png" style="zoom:80%;" />

所以，当建立了联合索引列后，能在where条件中使用索引的尽量使用。

#### 1.3.6.3.最佳左前缀法则

建立了联合索引列，如果搜索条件不够全值匹配怎么办？在我们的搜索语句中也可以不用包含全部联合索引中的列，但要遵守最左前缀法则。指的是**查询从索引的最左前列开始并且不跳过索引中的列。**
...
<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/6d37e1eeeac044c8935919a484dc12f4.png" alt="image.png" style="zoom: 80%;" />
...
**解释一下Extra中出现Using index**

意思是：**MySQL 只通过索引就能拿到需要的所有数据**，**不需要回表**。这就是传说中的 **覆盖索引（covering index）**。
...
当我们使用 **全表扫描**（也就是 `type = ALL`）来执行某条 SQL 查询，并且该语句中包含 **`WHERE` 条件**时，**MySQL 会在  `Extra` 列中显示 `Using where`**。
...
假设表 `users` 没有对 `age` 建立索引，那么：

- `type`: ALL（全表扫描）
- `Extra`: Using where

Extra中出现“Using where”，通常来说，意味着**全表扫描**或者在**部分使用索引**的情况下，但是还有查询条件不在索引字段当中。(博客园)



Extra` 中出现 `**Using index; Using where**

✅ 查询命中了 **覆盖索引**（`Using index`）

且

✅ 使用了 **WHERE 条件过滤**（`Using where`）

------

🚀 解读：

> 🔍 **数据只从索引中读取，无需回表**，但依然需要在服务器层应用 `WHERE` 条件进行过滤。

------

✅ 条件：

想要出现 `Using index; Using where`，必须满足两个条件：

1. **SELECT 的字段全部包含在索引中**
    👉 没有回表（覆盖索引）
2. **WHERE 条件中，至少部分条件需要服务器过滤**
    👉 不是所有过滤都在存储引擎层完成
...
```sql
EXPLAIN SELECT name, age FROM users WHERE name = 'Tom' AND age > 20;
```
...
| type  | key          | Extra                    |
| ----- | ------------ | ------------------------ |
| range | idx_name_age | Using index; Using where |

分析：

- ✅ 查询的字段 `name, age` 都在索引 `idx_name_age` 中
- ✅ `name='Tom'` 和 `age>20` 命中索引（但 `age>20` 是范围，不能用索引完全过滤）
- 所以：**不需要回表**（Using index） + **还需服务器层过滤**（Using where）

#### 1.3.6.7.Null/Not Null有影响
...
可以看见，order_no is null的情况下（根本没有结果），MySQL直接表示Impossible WHERE(**查询语句的WHERE子句永远为FALSE时将会提示该额外信息**，即永远不可能有结果)，对于 **is not null直接走的全表扫描**（因为全部都是非空）。
...
is null会走ref类型的索引访问，is not null的命中范围太大，依然是全表扫描（索引不如全表扫描划算）。所以总结起来：

is not null容易导致索引失效，统统走索引扫描；is null则会区分被检索的列是否允许为null，如果允许为null则会走ref类型的索引访问，如果不允许为null，也是全表扫描（但是根本没有结果）。
...
like以通配符开头('%abc...')，后缀匹配，MySQL索引失效会变成全表扫描的操作
...
MySQL的查询优化器，会自动的进行类型转换，比如**上个语句里会尝试将order_no转换为数字后和6进行比较，自然造成索引失效。**
...
这也给了我们提示，如果我们将 SQL改成union all，会有所改善。

```sql
explain 
SELECT * FROM order_exp WHERE expire_time= '2021-03-22 18:35:09' 
union all
SELECT * FROM order_exp WHERE order_note = 'abc';
```
...
FROM order_exp_cut 
WHERE insert_time='2021-03-22 18:34:55' or expire_time='2021-03-22 18:28:28';
```
...
MySQL有两种方式可以生成有序的结果﹔通过排序操作﹔或者按索引顺序扫描实现；如果EXPLAIN出来的type列的值为“index”，则说明MySQL使用了索引扫描来做排序。
...
- **对于使用联合索引进行排序的场景**，我们要求各个排序列的排序顺序是一致的，也就是各个列要么都是ASC规则排序，要么都是DESC规则排序。

- **排序列包含非同一个索引的列**

用来排序的多个列不是一个索引里的，这种情况也**不能使用索引进行排序。**

```sql
explain
SELECT * FROM order_exp 
order by order_no,insert_time;
```
...
注意到向**UUID主键插入行不仅花费的时间更长，而且索引占用的空间也更大**。这一方面是由于主键字段更长﹔另一方面毫无疑问是由于页分裂和碎片导致的。

因为主键的值是顺序的，所以InnoDB把每一条记录都存储在上一条记录的后面。当达到页的最大填充因子时(InnoDB默认的最大填充因子是页大小的15/16，**留出部分空间用于以后修改**)，**下一条记录就会写入新的页中**。一旦数据按照这种顺序的方式加载，主键页就会近似于**被顺序的记录填满**，这也正是所期望的结果。

如果新行的主键值不一定比之前插入的大，所以InnoDB无法简单地总是把新行插入到索引的最后，而是**需要为新的行寻找合适的位置**——通常是已有数据的中间位置——并且分配空间。这**会增加很多的额外工作**，并导致数据分布不够优化。下面是总结的一些缺点:

写入的目标页可能已经刷到磁盘上并从缓存中移除，或者是还没有被加载到缓存中，InnoDB在插入之前不得不先找到并从磁盘读取目标页到内存中。**这将导致大量的随机IO。**

因为写入是乱序的，InnoDB不得不频繁地做**页分裂操作**，以便为新的行分配空间。**页分裂会导致移动大量数据**，一次插入最少需要修改三个页而不是一个页。

**所以使用InnoDB时应该尽可能地按主键顺序插入数据，并且尽可能地使用单调增加的聚簇键的值来插入新行。**

#### 1.3.6.14.优化Count查询

首先要注意，COUNT()是一个特殊的函数，有两种非常不同的作用：**它可以统计某列不为 NULL 的行数，也可以统计表的总行数。**

**在统计某个列数时要求列值是非空的**（结果中也不统计NULL)。

COUNT()的另一个作用是统计结果集的行数。常用的就是就是当我们使用`COUNT(*)`。实际上，它会**忽略所有的列而直接统计所有的行数。**

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/3264f862df374302bef83c9459ad0a7a.png" alt="image.png" style="zoom:80%;" />
...
通常来说，`COUNT()`都需要扫描大量的行（意味着要访问大量数据）才能获得精确的结果，因此**是很难优化的**。在MySQL层面能做的基本只有索引覆盖扫描了。
...
```
 ...
 ✅ 因为只涉及 `status` 字段（在索引中），MySQL 可以直接扫描索引来做统计。

如果这还不够，就需要考虑修改应用的架构，可以用估算值取代精确值，可以增加汇总表，或者增加类似Redis这样的外部缓存系统。

#### 1.3.6.15.优化limit分页

在系统中需要进行分页操作的时候，我们通常会使用LIMIT加上偏移量的办法实现，同时加上合适的ORDER BY子句。

一个**非常常见又令人头疼的问题**就是，在**偏移量非常大**的时候，例如可能是
...
会先查询翻页中需要的N条数据的主键值，然后根据主键值回表查询所需要的N条数据，在此过程中查询N条数据的主键id在索引中完成，所以效率会高一些。下面的优化叫**延迟关联分页。**

就像在图书馆找书：

- 原始写法是：翻了 10,000 本书，每本都打开看内容；
- 延迟关联是：先查图书编号（轻量），然后只翻编号命中的那 10 本。
...
1、有的认为NULL值代表一个**未确定的值**，MySQL认为任何和NULL值做比较的表达式的值都为NULL，包括select
null=null和select null!=null;

<img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/11ebee5ff9db49b6bef2e80d2b2e46bd.png" alt="image.png" style="zoom:80%;" /><img src="https://fynotefile.oss-cn-zhangjiakou.aliyuncs.com/fynote/fyfile/5983/1651212459071/3e152ca5c31048a5afca4a7611bc5798.png" alt="image.png" style="zoom:80%;" />
...
- nulls_equal：**认为所有NULL值都是相等的**。这个值也是innodb_stats_method的**默认值。**


如果**某个索引列中NULL值特别多的话**，这种统计方式会让**优化器认为这个列中平均一个值重复次数特别多**，所以倾向于不选择使用该列的索引进行访问。

选择性低通常意味着该列的索引对于查询的帮助不大，因此优化器倾向于不使用索引。

- nulls_unequal：认为所有NULL值都是不相等的。


如果某个索引列中NULL值特别多的话，优化器会认为每个 `NULL` 值都视为不同的值，这样会增加该列的值的“不同性”（即选择性）。因此，优化器会认为该列的选择性更高，认为每个值（包括 `NULL`）的重复次数较少，**从而更倾向于使用索引来执行查询。**

- nulls_ignored：直接把NULL值忽略掉。


而且有迹象表明，在MySQL5.7.22以后的版本，对这个innodb_stats_method的修改不起作用，**MySQL把这个值在代码里写死为nulls_equal**。也就是说MySQL在进行索引列的数据统计行为又把null视为第二种情况，也就是nulls_equal（NULL值在业务上就是代表没有，所有的NULL值和起来算一份），看起来，MySQL中对Null值的处理也很分裂。**所以总的来说，对于列的声明尽可能的不要允许为null。**
