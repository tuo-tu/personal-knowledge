...
const (
	Key       byte = 0x01 // key 本身
	KeyData   byte = 0x02 // key 对应的 data
	End       byte = 0x04 // 结束标志
	HeaderLen      = 9
...
func (h *Header) Marshal() []byte {
	...
	b[0] = h.MessageType
	binary.BigEndian.PutUint64(b[1:], uint64(h.L))
	return b
}
...
func (r *Reader) Read(p []byte) (n int, err error) {
	for {
		...
		if r.isEnd {
			...
			// 本身已经结束了，并且buf没有数据，那么返回0, io.EOF
			return 0, io.EOF
		}
		...
		if _, err = r.conn.Read(headerData); err != nil {
			...
			// 注意这里是读取头信息，还没有读取body，而头信息在receive里面处理了。
			return 0, err
		}
...
func (conn *Conn) Receive() (key string, reader io.Reader, err error) {
	// 首先处理 Key 帧
	headerData := make([]byte, HeaderLen)
	if _, err = conn.conn.Read(headerData); err != nil {
		return "", nil, err
	}
	...
	for total < l {
		...
		n, err := conn.conn.Read(keyData[total : total+toRead])
		if err != nil {
			return "", nil, err
		}
		total += n
	...
	key = string(keyData)
	//fmt.Println("head =", header, "key =", key)
	reader = &Reader{
		conn:   conn.conn,
		buffer: []byte{},
	}
